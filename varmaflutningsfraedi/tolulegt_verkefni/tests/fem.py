...
def heatCond(mshfile, domain, boundary):
    ...
    for e in elem:
        ...
        ix = outer(e[:3],[1,1,1])
        k = domain[dlab]
        A[ix,ix.T] += k * (b @ b.T) / (4*S)
        volume[dlab] = volume.get(dlab, 0.0) + S
    ...
    for b in bc:
        ...
        A[ix,ix.T] += alpha * a * array([[1,2],[2,1]]) / 6
        rhs[b[:2]] -= beta * a * array([1,1]) / 2
    ...
    for b in bc:
        blab = lab[b[2]]
        r = mean(nodes[b[:2]][:,1])
        area = linalg.norm(nodes[b[0]]-nodes[b[1]])
